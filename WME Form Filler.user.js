...
(function () {
    ...
    function formfiller_init() {
        ...
        let userTabs = document.getElementById('user-tabs');
        if (userTabs === null) {
            window.setTimeout(formfiller_init, 500);
            return;
        }
        let navTab = userInfo.getElementsByTagName('ul');
        if (navTab.length === 0) {
            window.setTimeout(formfiller_init, 500);
            return;
        }
        ...
        let tabContent = userInfo.getElementsByTagName('div');
        ...
        ff_addUserTab();
        ff_addFormBtn();
        let formFillerObserver = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                ...
                for (let i = 0; i < mutation.addedNodes.length; i += 1) {
                    ...
                    if (addedNode.nodeType === Node.ELEMENT_NODE) {
                        let selectionDiv = addedNode.querySelector('div.selection');
        ...
        if (W.app.modeController) {
            W.app.modeController.model.bind('change:mode', (model, modeId) => {
                if (modeId === 0) {
                    ff_addUserTab();
                }
            });
            ...
            case 'closureStatus':
                if (selection[0].model.type === 'segment') {
                    if (ff_closureActive(selection)) {
                        ...
                        let closureInfo = ff_getClosureInfo(selection[0]);
                        ...
                        formValues.endDate = `${document.getElementById('ff-closure-endDate').value}+`
                            + `${document.getElementById('ff-closure-endTime').value}`;
                    }
                }
                break;
            default:
    ...
    function ff_addFormBtn() {
        ...
        var ffMnu = document.createElement('select');
        var ffBtn = document.createElement('button');
        var formWindowName = 'WME Form Filler result';
        var formWindowSpecs = 'resizable=1,menubar=0,scrollbars=1,status=0,toolbar=0';
        var editPanel;
        var selElem;
        var formLink;
    ...
    function ff_loadSettings() {
        ...
        if (Object.prototype.hasOwnProperty.call(localStorage, settingsKey)) {
            ...
            if (ffClosureEndDate !== null && ffClosureEndDate !== '' && ffClosureEndDate >= today) {
                ...
                let closureDate = `${futureDate.getFullYear()}-`
    ...
    function ff_addUserTab() {
        ...
        var ffReason = document.createElement('div');
        var ffEndDate = document.createElement('div');
        var ffNewTabBox = document.createElement('input');
        var ffNewTabLabel = document.createElement('label');
        var ffTabInfo = document.createElement('div');

        ffTab.innerHTML = '<a title="Form Filler" href="#sidepanel-formfill" data-toggle="tab">'
            + `<img class="fa" src="${scriptIcon}" width="15px" /></a>`;
        ...
        ffReason.innerHTML = '<label class="control-label" for="ff_closure_reason">Closures reason:</label>'
            + '<div class="controls">'
            + '<input id="ff-closure-reason" class="form-control" type="text" name="ff_closure_reason"></div>';

        ffEndDate.className = 'form-group';
        ffEndDate.innerHTML = '<label class="control-label" for="ff_closure_endDate">Closures end:</label>'
            + '<div class="controls"><div class="date date-input-group input-group pull-left" style="width: 62%">'
            + '<input id="ff-closure-endDate" class="form-control end-date" type="text" name="ff_closure_endDate">'
            + '<span class="input-group-addon"><i class="fa fa-calendar"></i></span></div>'
            + '<div class="bootstrap-timepicker input-group style="width: 38%"">'
            + '<input id="ff-closure-endTime" class="end-time form-control" type="text" name="ff_closure_endTime">'
        ...
        ff_loadSettings();
        $('#ff-closure-reason').change(() => {
            ff_saveSettings();
        });
        $('#ff-closure-endDate').change(() => {
            ff_saveSettings();
        });
        $('#ff-closure-endTime').change(() => {
            ff_saveSettings();
        });
        $('#ff-open-in-tab').click(() => {
            ff_saveSettings();
        });
    }
